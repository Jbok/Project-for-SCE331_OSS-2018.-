...
<br><br><br>
야구 경기가 중계되는 전체화면 중에서 선수의 이름이 있는 위치는 일정하다. (단, 야구 중계 채널마다 선수의 이름의 위치가 다르기 때문에 [좌표값][1]이 상이하다) <br>
따라서 **코드 상에서 그 위치를 지정**해준다면 생중계되는 화면에서 선수의 이름을 인식하는 과정을 좀 더 쉽게 처리할 수 있을 것이라고 기대해볼 수 있다. <br>

현재 OpenCV와 Tesseract 두 오픈소스를 사용하여 
