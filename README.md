...
1. **OpenCV**로 영상과 이미지 프로세싱이 동시에 가능하다. OpenCV는 영상 관련 라이브러리로서 사실상 표준의 지위를 가지고 있으며, 영상처리를 대중화시킨 라이브러리이다.<br>
2. **Tesseract**의 경우 영어 뿐만 아니라 한글 또한 인식 능력이 뛰어나다. Google이 지원하고 있으며, 오픈소스 OCR 중에는 가장 널리 사용되고 있다.<br>
...
<br>
Tesseract와 OpenCV 모두 오픈소스이기 때문에 여러 개발자들에 의해 각각의 성능이 개선 될 수 있다.<br> 
그렇게 된다면 더 나은 영상처리와 문자인식을 보여줄 것이고, 따라서 두 오픈소스를 합쳐 개발하는 이 오픈소스의 성능 역시 향상될 가능성이 존재한다.<br>
<br>
또한, 우리 팀이 최초로 기획했던 스포츠 중계에서의 적용 뿐만 아니라 다른 분야에서도 응용될 수 있다.<br>
예를 들어, 웹 브라우저의 확장기능의 형태로 영상의 문자 인식 기능을 활용하여 여러가지 서비스를 개발할 수 있다. <br>
사용자가 게임과 관련된 영상을 재생할 경우 자막을 인식하면 게임과 관련된 정보를 보여준다거나, 음악방송을 재생할 경우 해당 가수에 대한 정보를 제공하는 것이다. <br>
이처럼, 개발자는 이용자가 재생하는 임의의 영상의 종류에 따라서 글자인식을 활용하여 무궁무진한 서비스를 개발하여 제공할 수 있을 것이다. <br>
<br>
...
4. 실시간 야구 중계 등 실질적 목적인 스트리밍 서비스 이용 환경에 문자 인식 기능을 어떻게 접목시킬 것인지
<br>
